...
public class MiscService extends BaseService{
	...
	public Response<ThaaliCook> getCook(@QueryParam("ejamaatId")String eJamaatId, @QueryParam("password")String password){
		String msg = "";
		boolean isError = false;
		List<ThaaliCook> thaaliCooks = null;
		try{
			authenticateUser(eJamaatId, password);			
			ThaaliCookDAO thaaliCookDAO = new ThaaliCookDAOImpl();
			thaaliCooks = thaaliCookDAO.getCooks();
			if(thaaliCooks == null || thaaliCooks.isEmpty()){
				isError = true;
				msg = "No cooks have been added yet.";
			}
			
		}catch(Exception ex){
			msg = "An exception has occurred inside getCook for the i/p eJamaatId - "+eJamaatId+",password -"+password;
			Logger.error(COMP_NAME, msg, ex);
			isError = true;
		}
		return new Response<ThaaliCook>(thaaliCooks, msg, isError);
	}
