...
public class UserThaaliDataService extends BaseService{
		...
		try {
			...
			if(!thaaliDataResponse.isError()){
				...
				if(!Utils.isNullOrEmpty(thaaliDataList)){
					...
					//We know for sure that Thaali is present for duration specified.
				   ...
				   //Now lets get the UserThaaliData for the startDate and endDate that we got from above.
					...
					for(int i=0;i<thaaliDataList.size();i++){
						...
						boolean isThresholdReached = isThresholdReached(thaaliDate);
						...
						userThaaliView.setLocked(isThresholdReached);
	...
	public Response<UserThaaliData> updateUserThaaliData(Request<UserThaaliData> requestObj){
		...
		try{
			...
			List<ThaaliData> thaaliDataList = thaaliDataDAO.getThaaliData2(fromDate, toDate, Boolean.TRUE);
				...
				for(int i=0;i<size;i++){
					...
					for(int j=0;j<thaaliDataList.size();j++){
						...
						if(userThaaliDate.compareTo(thaaliDate) == 0){
							if(ThaaliStatus.THAALI_PRESENT.equals(thaaliData.getStatus())){	
								...
								if(!isThresholdReached(thaaliDate)){
									...
									//Update did not happen successfully.
									 logMsg = "User Thaali Data did not update/create successfully since no more modifications are allowed for this day.";
									isError = true;
								}
							}else{
								//Update did not happen successfully.
								logMsg = "User Thaali Data did not update/create successfully since there is no thaali present on this day";
								isError = true;
							}
